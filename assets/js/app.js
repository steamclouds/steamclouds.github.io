        ...
        ${
          a
            ? `<a class="btn btn--primary"
                 href="#"
                 data-action="lv-redirect"
                 data-url="${a.browser_download_url}">
      ...
      ${
        a
          ? `<a class="btn btn--ghost"
               href="#"
               data-action="lv-redirect"
               data-url="${a.browser_download_url}">Download</a>`
          : ''
      }
    </li>`;
  }).join('');
...
function getLogsForTool(toolIndex){
  ...
  if (logsData && !Array.isArray(logsData) && typeof logsData === 'object') {
    ...
    const byTitle = logsData[toolsData[toolIndex]?.title]
                 ?? logsData[title]
                 ?? null;
  ...
  : '';
    ...
    return `
      <article class="tool" data-tool-index="${i}" data-reveal>
        ${badge}
        <div class="tool__media"><img class="tool__img" src="${t.img}" alt="${escapeHtml(t.title)}" loading="lazy"/></div>
...
function reinitLV(){
  if (typeof window.linkvertise === 'function') {
    window.linkvertise(1202082, {
      whitelist: [],
      blacklist: ['steamclouds.online','www.steamclouds.online']
    });
  }
}

function openViaLinkvertise(url){
  try {
    const host = new URL(url, location.href).hostname;
    if (host === 'steamclouds.online' || host === 'www.steamclouds.online') {
      location.href = url;
      return;
    }
  } catch {}

  const a = document.getElementById('lv-proxy');
  ...
  gridRoot.addEventListener('click', async (e)=>{
    ...
    if (action === 'download') {
      ...
      if (direct) {
          ...
          if (href) openViaLinkvertise(href, 'Download');
          return;
        }
      ...
      try {
        ...
        openViaLinkvertise(a.browser_download_url, a.name || 'Download');
      } catch (err) {
        console.error(err);
...
document.addEventListener('click', (e) => {
  const a = e.target.closest('[data-action="lv-redirect"]');
  if (!a) return;
  e.preventDefault();
  const url = a.getAttribute('data-url');
  if (url) openViaLinkvertise(url, a.textContent || 'Download');
});
